...
describe("Add correctly adds", () => {
  test("number, number", () => {
    const value = 1;
    const expected = 2;
    const result = add({ value, toAdd: value });
    expect(result).toBe(expected);
  });

  test("ImperialValue,ImperialValue", () => {
    const value: ImperialValue = { ft: 1, ins: 1, fr: fraction(1, 3) };
    const toAdd: ImperialValue = { ft: 1, ins: 1, fr: fraction(1, 6) };
    const result = add({ value, toAdd });
    expect(JSON.stringify(result)).toBe(
      JSON.stringify({ ft: 2, ins: 2, fr: fraction(1, 2) })
    );
  });
