...
export class Fraction {
  ...
  static fromDecimal = (decimal: number) => {
    ...
    let denominator = Math.pow(
      10,
      denominatorArray[denominatorArray.length - 1].length,
    );
  ...
  static closestTapeMeasure = (
    decimal: number,
    resolution: Resolution = Resolution.thirtySecond,
  ...
  add = (other: Fraction) => {
    ...
    let newN: number =
      (lcm / this.d) * (this.n ?? 0) + (lcm / other.d) * (other.n ?? 0);
